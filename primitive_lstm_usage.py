...
from primitive_lstm import PrimitiveLSTMBackbone

# USAGE FILE
# This file does not define the primitive LSTM architecture itself.
...
class PrimitiveCharLSTMLanguageModel(nn.Module):
    # Equation set (B1) in primitive_lstm_usage.md:
    #   e_t = Embedding(x_t)
    #   h_t = PrimitiveLSTM(e_t, h_{t-1}, c_{t-1})
    #   z_t = h_t W_hy + b_y
    ...
    def __init__(self, vocab_size: int, embed_dim: int, hidden_dim: int) -> None:
        ...
        self.backbone = PrimitiveLSTMBackbone(embed_dim, hidden_dim)  # Equation (B1): h_t = PrimitiveLSTM(e_t, h_{t-1}, c_{t-1}).
...
def generate_sample_text(
    model: PrimitiveCharLSTMLanguageModel,
...
def write_primitive_lstm_report(
    report_path: Path,
    ...
    lines = [
        "# Primitive LSTM Report",
        "",
        "This report is produced by `primitive_lstm_usage.py` for the architecture in `primitive_lstm.py`.",
        "",
        "## Run Summary",
        "",
    ...
    lines.extend(
        [
            ...
            "```",
            "",
            "## Why LSTM Often Holds Context Better",
            "",
            "- The cell state `c_t` gives the model a dedicated memory path across time.",
            "- The forget gate `f_t` controls what old memory is kept.",
            "- The input gate `i_t` controls what new information is written.",
            "- The output gate `o_t` controls how much memory is exposed as hidden state.",
            "- Those gates often make LSTM less prone to short repetitive loops than primitive RNN.",
            "",
            "## Interpretation",
            "",
            "- This report belongs to the primitive LSTM architecture only.",
            "- The architecture equations are documented in `primitive_lstm.md`.",
            "- The training workflow equations are documented in `primitive_lstm_usage.md`.",
...
def train_until_test_reverses() -> None:
    # Loads the dataset used for the primitive-LSTM training demonstration.
    dataset = build_sequence_dataset(Path("tinyshakespeare.txt"))
    # Instantiates the language model from Equation set (B1).
    model = PrimitiveCharLSTMLanguageModel(vocab_size=len(dataset.stoi), embed_dim=32, hidden_dim=64)
    ...
    checkpoint_path = Path("primitive_lstm_best_checkpoint.pt")  # Stores theta* from the best test epoch.
    report_path = Path("primitive_lstm_report.md")  # Stores a human-readable report for this architecture run.
