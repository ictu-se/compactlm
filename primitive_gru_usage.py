...
# USAGE FILE
# This file does not define the primitive GRU architecture itself.
...
def generate_sample_text(
    model: PrimitiveCharGRULanguageModel,
...
def write_primitive_gru_report(
    report_path: Path,
    ...
    lines = [
        "# Primitive GRU Report",
        "",
        "This report is produced by `primitive_gru_usage.py` for the architecture in `primitive_gru.py`.",
        "",
        "## Run Summary",
        "",
    ...
    lines.extend(
        [
            ...
            "```",
            "",
            "## Why GRU Often Repeats Less Than Primitive RNN",
            "",
            "- The update gate `z_t` helps the model decide when new information should replace old information.",
            "- The reset gate `r_t` helps the model control how much past state is used in the candidate update.",
            "- Those gates make it easier to preserve useful context over more time steps.",
            "- Even so, greedy decoding can still loop if one continuation becomes locally dominant.",
            "",
            "## Interpretation",
            "",
            "- This report belongs to the primitive GRU architecture only.",
            "- The architecture equations are documented in `primitive_gru.md`.",
            "- The training workflow equations are documented in `primitive_gru_usage.md`.",
...
def train_until_test_reverses() -> None:
    # Loads the dataset used for the primitive-GRU training demonstration.
    dataset = build_sequence_dataset(Path("tinyshakespeare.txt"))
    # Instantiates the language model from Equation set (B1).
    model = PrimitiveCharGRULanguageModel(vocab_size=len(dataset.stoi), embed_dim=32, hidden_dim=64)
    ...
    checkpoint_path = Path("primitive_gru_best_checkpoint.pt")  # Stores theta* from the best test epoch.
    report_path = Path("primitive_gru_report.md")  # Stores a human-readable report for this architecture run.
